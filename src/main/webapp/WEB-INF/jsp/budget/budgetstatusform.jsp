	...
	var columnDefs = [
	  ...
	  gridOptions = {
	    		  ...
	    		  onRowClicked:function (event){
	    			  ...
	    			  selectedRow=event.data;
	    			  var array=[];
	    			  console.log(array.push(selectedRow));
	    			  gridOptions2.api.setRowData(array);
	    		  }
	   }
	...
	$.ajax({
        ...
        success: function (jsonObj) {
        	...
        	for(var i=1; i<=12;i++){
        		...
        		if(jsonObj.budgetBean)
        			if(jsonObj.budgetBean["m"+i+"Budget"]||jsonObj.budgetBean["m"+i+"Budget"]==0){
        		var value=jsonObj.budgetBean["m"+i+"Budget"]+"";//numToMoney 함수 적용하기
        		input.value=numToMoney(value);
        		num+=jsonObj.budgetBean["m"+i+"Budget"];
        		if(i%3==0){
        			var q=document.querySelector("#q"+i/3);
